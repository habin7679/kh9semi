...
// 		list = boardDao.selectList(type, keyword);
		list = boardDao.selectTipListByPaging(p, s, type, keyword); 
	}
	else {
// 		list = boardDao.selectList();
		list = boardDao.selectTipListByPaging(p, s);
	}
%>    
...
<%
	...
	if(search){//검색 결과 수 카운트
		count = boardDao.countByPaging_tip(type, keyword);
	}
	else{//목록 결과 수 카운트
		count = boardDao.countByPaging_tip();
	}
...
<%-- 					<td><%=boardDto.getDepth()%></td> --%>
	...
	<div class="row center pagination">
		...
		<%if(p > 1){ %>
			<%if(search){ %>
			<a href="tip_list.jsp?p=1&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>">&laquo;</a>
			<%} else { %>
			<a href="tip_list.jsp?p=1&s=<%=s%>">&laquo;</a>
			<%} %>
		<%} %>
		
		<%if(startBlock > 1){ %>
			<%if(search){ %>
			<a href="tip_list.jsp?p=<%=startBlock-1%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>">&lt;</a>
			<%} else { %>
			<a href="tip_list.jsp?p=<%=startBlock-1%>&s=<%=s%>">&lt;</a>
			<%} %>
		<%} %>
		...
		<%for(int i=startBlock; i <= endBlock; i++){ %>
			<%if(search){ %>
				<%if(i == p){ %>
				<a class="active" href="tip_list.jsp?p=<%=i%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>"><%=i%></a>	
				<%} else { %>
				<a href="tip_list.jsp?p=<%=i%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>"><%=i%></a>
				<%} %>
			<%} else { %>
				<%if(i == p){ %>
				<a class="active" href="tip_list.jsp?p=<%=i%>&s=<%=s%>"><%=i%></a>	
				<%} else { %>
				<a href="tip_list.jsp?p=<%=i%>&s=<%=s%>"><%=i%></a>
				<%} %>
			<%} %>
		<%} %>
		...
		<%if(endBlock < lastPage){ %>
			<%if(search){ %>
			<a href="tip_list.jsp?p=<%=endBlock+1%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>">&gt;</a>
			<%} else { %>
			<a href="tip_list.jsp?p=<%=endBlock+1%>&s=<%=s%>">&gt;</a>
			<%} %>
		<%} %>
		
		<%if(p < lastPage){ %>
			<%if(search){ %>
			<a href="tip_list.jsp?p=<%=lastPage%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>">&raquo;</a>
			<%} else { %>
			<a href="tip_list.jsp?p=<%=lastPage%>&s=<%=s%>">&raquo;</a>
			<%} %>
		<%} %>
	...
	<div class="row center">
		<!-- 검색창 -->
		<form action="tip_list.jsp" method="get">
